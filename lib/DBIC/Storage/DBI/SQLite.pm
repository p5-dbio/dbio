package DBIC::Storage::DBI::SQLite;
...
use DBIC::_Util qw(modver_gt_or_eq sigwarn_silencer);
use DBIC::Carp;
use Try::Tiny;
use namespace::clean;

__PACKAGE__->sql_maker_class('DBIC::SQLMaker::SQLite');
...
DBIC::Storage::DBI::SQLite - Automatic primary key class for SQLite
...
=item RT79576
  ...
  * You are or were using DBIC versions between 0.08191 and 0.08209
...
An unfortunate chain of events led to DBIC silently hitting the problem
...
DBIC call to C<< $sth->bind_param >> would be executed in a way that
...
Even if you upgrade DBIC (which works around the bug starting from
version 0.08210) you may still have corrupted/incorrect data in your database.
DBIC warned about this condition for several years, hoping to give
...
sub _dbh_execute {
  if (
    (
      DBIC::_ENV_::IV_SIZE < 8
        or
      DBIC::_ENV_::OS_NAME eq 'MSWin32'
    )
      and
...
In L<connect_info|DBIC::Storage::DBI/connect_info> to turn on foreign key
