...
use Clone;
use DBIO::CDBICompat::Relationship;
use DBIO::_Util qw(quote_sub perlstring);
...
DBIO::CDBICompat::Relationships - Emulate has_a(), has_many(), might_have() and meta_info()
...
sub _declare_has_a {
  ...
  if( !$f_class->isa('DBIO::Row') and !$f_class->isa('Class::DBI::Row') ) {
...
sub _extend_meta {
    ...
    $hash{$type}{$rel} = DBIO::CDBICompat::Relationship->new($val);
