package DBIO::Loader::DBI;
# ABSTRACT: DBI-based introspection base class

use strict;
use warnings;
use base qw/DBIO::Loader::Base/;
use mro 'c3';
use Try::Tiny;
use List::Util 'any';
...
DBIO::Loader::DBI - DBIO::Loader DBI Implementation.

=head1 SYNOPSIS

See L<DBIO::Loader::Base>

=head1 DESCRIPTION

This is the base class for L<DBIO::Loader::Base> classes for
DBI-based storage backends, and implements the common functionality between them.

See L<DBIO::Loader::Base> for the available options.

=head1 METHODS

=head2 new

Overlays L<DBIO::Loader::Base/new> to do some DBI-specific
things.
...
sub new {
    ...
    if (not $self->loader_class) {
        ...
        my %driver_loader = (
            ...
            ODBC    => 'DBIO::Loader::ODBC',
        );

        my $subclass = $driver_loader{$driver};

        # Fallback: try driver-namespaced convention for unknown drivers
        if (!$subclass) {
            # e.g. Firebird -> DBIO::Firebird::Loader
            $subclass = 'DBIO::' . $driver . '::Loader';
        }
...
sub _tables_list {
    ...
    foreach my $schema (@{ $self->db_schema || [undef] }) {
        ...
        TABLE: foreach my $raw_table_name (@raw_table_names) {
            ...
            # in most cases. Bug reports welcome on GitHub.
            ...
            my $table = DBIO::Loader::Table->new(
                loader => $self,
                name   => $table_name,
                schema => $schema_name,
...
We override L<DBIO::Loader::Base/load> here to hook in our localized settings for C<$dbh> error handling.
...
sub _table_fk_info {
    ...
    REL: while(my $raw_rel = $sth->fetchrow_arrayref) {
        ...
        $rels{$relid}{tbl} ||= DBIO::Loader::Table->new(
            loader => $self,
            name   => $uk_tbl,
            schema => $uk_scm,
            ($self->_supports_db_schema ? () : (
...
# ported in from DBIO::Storage::DBI:
