package DBIO::Loader::Base;
# ABSTRACT: Core introspection engine for DBIO::Loader
...
DBIO::Loader::Base - Base DBIO::Loader Implementation.
...
This is the base class for the storage-specific C<DBIO::Schema::*>
classes, and implements the common functionality between them.

=head1 CONSTRUCTOR OPTIONS

These constructor options are the base options for
L<DBIO::Loader/loader_options>.  Available constructor options are:
...
existing dump made by an older version of L<DBIO::Loader>, in
...
use this to turn L<DBIO> cascades to on on your
L<has_many|DBIO::Relationship/has_many> and
L<might_have|DBIO::Relationship/might_have> relationships, they default
to off.
...
this hash of parameters (as a list):
    ...
    local_source    # the DBIO::ResultSource object for the source the rel is *from*
    remote_source   # the DBIO::ResultSource object for the source the rel is *to*
    local_table     # the DBIO::Loader::Table object for the table of the source the rel is from
    local_cols      # an arrayref of column names of columns used in the rel in the source it is from
    remote_table    # the DBIO::Loader::Table object for the table of the source the rel is to
...
For L<belongs_to|DBIO::Relationship/belongs_to> relationships, these
...
values for a L<belongs_to|DBIO::Relationship/belongs_to> relationship
will be:
...
value is set to C<1> if L<DBIO> has a working C<<
$storage->with_deferred_fk_checks >>. This is done so that the same
L<DBIO> code can be used, and cross deployed from and to such databases.

=head2 debug

If set to true, each constructive L<DBIO> statement the loader
...
L<DBIO::Loader::Table> class in the loader. Some drivers provide
subclasses (e.g. L<DBIO::Sybase::Loader::Table>,
L<DBIO::Informix::Loader::Table>) for database-specific handling.

Monikers are created normally based on just the
L<name|DBIO::Loader::DBObject/name> property, corresponding to
...
the L<DBIO::Loader::Table> object for the table
...
the L<DBIO::Loader::Column> object for the column
...
a hashref of this form:

    {
        ...
        table           => the DBIO::Loader::Table object for the table,
...
Base class for your schema classes. Defaults to 'DBIO::Schema'.
...
L<InflateColumn::DateTime|DBIO::InflateColumn::DateTime>
...
This is now the default, to go back to L<DBIO::Schema/load_classes> pass
a C<0>.

Generate result class names suitable for
L<DBIO::Schema/load_namespaces> and call that instead of
L<DBIO::Schema/load_classes>. When using this option you can also
...
L<DBIO::Schema> module set, based on what it creates at runtime.
...
See L<DBIO::Loader/dump_to_dir> for examples of the
...
L<column_info|DBIO::ResultSource/column_info> for a column.

Must be a coderef that returns a hashref with the extra attributes.

Receives the L<DBIO::Loader::Table> object, column name
and column_info.
...
Sets the timezone attribute for L<DBIO::InflateColumn::DateTime> for all
columns with the DATE/DATETIME/TIMESTAMP data_types.

=head2 datetime_locale

Sets the locale attribute for L<DBIO::InflateColumn::DateTime> for all
columns with the DATE/DATETIME/TIMESTAMP data_types.
...
users of L<DBIO::Loader>. Some are proxied via
L<DBIO::Loader>.
...
sub new {
    ...
    croak "dump_overwrite is deprecated.  Please read the"
        . " DBIO::Loader::Base documentation"
    ...
    $self->version_to_dump($DBIO::Loader::VERSION);
    $self->schema_version_to_dump($DBIO::Loader::VERSION);
...
See perldoc DBIO::Loader::Manual::UpgradingFromV4 for more
details.
EOF
        }
        else {
...
See: 'naming' in perldoc DBIO::Loader::Base .

See perldoc DBIO::Loader::Manual::UpgradingFromV4 if upgrading
...
sub _validate_classes {
    ...
    foreach my $c (@classes) {
        # components default to being under the DBIO namespace unless they
        # are preceded with a '+'
    ...
    for my $table (@tables) {
...

        {
            ...
            if (
                ...
                && ($resultset_class ne 'DBIO::ResultSet')
            ) {
...
sub _dump_to_dir {
    ...
    my $schema_base_class = $self->schema_base_class || 'DBIO::Schema';
    ...
    my $result_base_class = $self->result_base_class || 'DBIO::Core';
...
sub _parse_generated_file {
    ...
    while(<$fh>) {
        if(/$mark_re/) {
            ...
            ($ver, $ts) = $gen =~ m/^# Created by DBIO::Loader( v[\d._]+)?( @ [\d-]+ [\d:]+)?\r?\Z/m;
            $ver =~ s/^ v// if $ver;
            $ts =~ s/^ @ // if $ts;
...
sub _is_result_class_method {
    ...
    if (not exists $self->_result_class_methods->{$table_moniker}) {
        my (@methods, %methods);
        my $base       = $self->result_base_class || 'DBIO::Core';
...
See "COLUMN ACCESSOR COLLISIONS" in perldoc DBIO::Loader::Base .
EOF
                $info->{accessor} = undef;
            }
        }
...
sub _setup_src_meta {
    ...
    $self->_dbic_stmt($table_class, 'table_class', 'DBIO::ResultSource::View')
    ...
    while (my ($col, $info) = each %$col_info) {
        ...
        my $col_obj = DBIO::Loader::Column->new(
            table => $table,
            name  => $col,
        );
