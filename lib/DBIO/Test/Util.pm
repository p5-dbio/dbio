...
use constant DEBUG_TEST_CONCURRENCY_LOCKS =>
  ( ($ENV{DBIOTEST_DEBUG_CONCURRENCY_LOCKS}||'') =~ /^(\d+)$/ )[0]
    ||
  0
;
...
if (DEBUG_TEST_CONCURRENCY_LOCKS) {
  ...
  *DBI::connect = sub {
    DBIO::Test::Util::dbg("Connecting to $_[1]");
    goto $oc;
  }
}
...
sub await_flock ($$) {
  ...
  while(
    ...
    unless ( $tries % 10 ) {
      ...
      #
      # DBIOTEST_VERSION_WARNS_INDISCRIMINATELY=1 \
      # DBIOTEST_RUN_ALL_TESTS=1 \
      # strace -f \
...
sub check_customcond_args ($) {
  ...
  confess "Custom condition argument 'self_resultsource' must be a rsrc instance"
    unless defined blessed $args->{self_resultsource} and $args->{self_resultsource}->isa('DBIO::ResultSource');
  ...
  if (defined $args->{self_result_object} or defined $args->{self_rowobj} ) {
    ...
    for (qw(self_result_object self_rowobj)) {
      confess "Custom condition argument '$_' must be a result instance"
        unless defined blessed $args->{$_} and $args->{$_}->isa('DBIO::Row');
    }
