...
Columns marked with C<< changelog =E<gt> 0 >> are omitted from changelog
entries for insert and delete events, and changes to these columns are
not recorded in update events.

Changelog entries are written to a per-source C<< <table>_changelog >>
table that is auto-generated by L<DBIO::Schema::ChangeLog>.

=head1 DRIVER OVERRIDE HOOKS

Drivers (e.g. L<DBIO::PostgreSQL::ChangeLog>) can override the
C<changelog_*> methods via C<next::method> to customize storage formats,
write paths, and notification.
