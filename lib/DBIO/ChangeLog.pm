...
A component that automatically tracks insert, update, and delete
operations on a Result class.  Load it via C<load_components('ChangeLog')>
on any Result class whose changes you want to record.
...
Drivers (e.g. L<DBIO::PostgreSQL::ChangeLog>) can override these
methods via C<next::method> to customize storage formats, write paths,
and notification.
...
__PACKAGE__->mk_classdata('_changelog_exclude_columns' => {});

=method changelog_exclude_columns

  __PACKAGE__->changelog_exclude_columns(qw/ password_hash secret /);

Class-level accessor.  Columns listed here are omitted from changelog
entries for insert and delete events, and changes to these columns are
not recorded in update events.

=cut

sub changelog_exclude_columns {
  my $class = shift;
  if (@_) {
    $class->_changelog_exclude_columns({ map { $_ => 1 } @_ });
  }
  return keys %{ $class->_changelog_exclude_columns };
}
...
sub _changelog_filtered_columns {
  my ($self, %cols) = @_;
  my $exclude = $self->_changelog_exclude_columns;
