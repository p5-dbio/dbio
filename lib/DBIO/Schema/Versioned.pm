...
L<DBIO::DeploymentHandler> is a much more powerful alternative to this
...
L<DBIO::Schema/ddl_filename>) then create a diff using your create DDL
...
Same as L<DBIO::Schema/deploy> but also calls C<install>.
...
sub _on_connect
{
  ...
  $self->{vschema} = DBIO::Version->connect(
    sub { $w_storage->dbh },
...
sub _create_db_to_schema_diff {
  ...
  unless (DBIO::Optional::Dependencies->req_ok_for ('deploy')) {
    $self->throw_exception("Unable to proceed without " . DBIO::Optional::Dependencies->req_missing_for ('deploy') );
  }
  ...
  $dbic_tr->parser('SQL::Translator::Parser::DBIO');
  $dbic_tr->data($self);
  $dbic_tr->producer($db);
