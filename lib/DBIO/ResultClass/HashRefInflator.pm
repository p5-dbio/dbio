...
DBIO is faster than older ORMs like Class::DBI but it still isn't
...
$mk_hash = sub {
  ...
  my $hash = {
    ...
    map { $_ => (
      ...
      # null-branch or not
      ref $_[1]->{$_} eq $DBIO::ResultSource::RowParser::Util::null_branch_class
...
following:
 ...
 my $cds = $artist->cds;
 $cds->result_class('DBIO::ResultClass::HashRefInflator');
 my $first = $cds->first;
...
L<DBIO::InflateColumn::DateTime>, is not performed.
