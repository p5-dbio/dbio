...
DBIO::UTF8Columns - Force UTF8 (Unicode) flag on columns (DEPRECATED)
...
stored and de-serialised to unicode strings on retrieval.
  ...
  THE USE OF THIS MODULE (AND ITS COUSIN DBIO::ForceUTF8) IS VERY
...
Recently (April 2010) a bug was found deep in the core of L<DBIO>
which affects any component attempting to perform encoding/decoding by
overloading L<store_column|DBIO::Row/store_column> and
L<get_columns|DBIO::Row/get_columns>. As a result of this problem
L<create|DBIO::ResultSet/create> sends the original column values
to the database, while L<update|DBIO::ResultSet/update> sends the
encoded values. L<DBIO::UTF8Columns> and L<DBIO::ForceUTF8>
...
implications of changing such a fundamental behavior of DBIO are
...
L<join us on IRC or the mailing list|DBIO/GETTING HELP/SUPPORT>
...
Note that this module overloads L<DBIO::Row/store_column> in a way
that may prevent other components overloading the same method from working
correctly. This component must be the last one before L<DBIO::Row>
(which is provided by L<DBIO::Core>). DBIO will detect such
incorrect component order and issue an appropriate warning, advising which
components need to be loaded differently.

=head1 SEE ALSO

L<Template::Stash::ForceUTF8>, L<DBIO::UUIDColumns>.
