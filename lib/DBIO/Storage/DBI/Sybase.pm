...
sub _init {
  ...
  my $self = shift;
  if (! $self->isa('DBIO::Storage::DBI::Sybase::FreeTDS') and $self->_using_freetds) {
    require DBIO::Storage::DBI::Sybase::FreeTDS;
    ...
    no strict 'refs';
    @{"${trait_class}::ISA"} = ($class, 'DBIO::Storage::DBI::Sybase::FreeTDS', @isa);
