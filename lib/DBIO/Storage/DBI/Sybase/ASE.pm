...
be reblessed to L<DBIO::Storage::DBI::Sybase::ASE::NoBindVars>.
...
A recommended L<connect_info|DBIO::Storage::DBI/connect_info> setting:
...
See perldoc DBIO::Storage::DBI::Sybase::ASE for more details.
...
As of version C<0.05000>, L<DBIO::Schema::Loader> should work well with
...
L<DBIO::Storage::DBI::Sybase::ASE::NoBindVars>.
...
the C<auto_cast> option from L<DBIO::Storage::DBI::AutoCast> is
automatically set, which you may enable on connection with
L<connect_call_set_auto_cast|DBIO::Storage::DBI::AutoCast/connect_call_set_auto_cast>.
The type info for the C<CAST>s is taken from the
L<DBIO::ResultSource/data_type> definitions in your Result classes, and
...
When using C<DBIO::Storage::DBI::Sybase::ASE::NoBindVars> transactions
...
L<ResultSet|DBIO::ResultSet> that has been executed using C<next> or
C<first> but has not been exhausted or L<reset|DBIO::ResultSet/reset>.
...
=item * use L<DBIO::Storage::DBI::Replicated>

=item * L<connect|DBIO::Schema/connect> another L<Schema|DBIO::Schema>

=item * load the data from your cursor with L<DBIO::ResultSet/all>
...
This is a bit too low for a complex L<DBIO> application, so on connection
...
for L<DBIO::InflateColumn::DateTime>.
...
L<GenericSubQ|DBIO::SQLMaker::LimitDialects/GenericSubQ>.

Fortunately, ASE and L<DBD::Sybase> support cursors properly, so when
L<GenericSubQ|DBIO::SQLMaker::LimitDialects/GenericSubQ> is too slow
you can use the L<software_limit|DBIO::ResultSet/software_limit>
L<DBIO::ResultSet> attribute to simulate limited queries by skipping
over records.
...
L<connect_info|DBIO::Storage::DBI/connect_info> is used to execute the
equivalent C<SET TEXTSIZE> command on connection.

See L</connect_call_blob_setup> for a
L<connect_info|DBIO::Storage::DBI/connect_info> setting you need to work
with C<IMAGE> columns.

=head1 BULK API

The experimental L<DBD::Sybase> Bulk API support is used for
L<populate|DBIO::ResultSet/populate> in B<void> context, in a transaction
on a separate connection.

To use this feature effectively, use a large number of rows for each
L<populate|DBIO::ResultSet/populate> call, eg.:

  while (my $rows = $data_source->get_100_rows()) {
    $rs->populate($rows);
  }

B<NOTE:> the L<add_columns|DBIO::ResultSource/add_columns>
...
L<DBIO::InflateColumn::DateTime>, and most people do, and still want to
