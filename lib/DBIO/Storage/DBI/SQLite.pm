...
__PACKAGE__->sql_maker_class('DBIO::SQLMaker::SQLite');
...
=item RT79576
  ...
  * You are or were using DBIO versions between 0.08191 and 0.08209
...
An unfortunate chain of events led to DBIO silently hitting the problem
...
DBIO call to C<< $sth->bind_param >> would be executed in a way that
...
Even if you upgrade DBIO (which works around the bug starting from
version 0.08210) you may still have corrupted/incorrect data in your database.
DBIO warned about this condition for several years, hoping to give
...
In L<connect_info|DBIO::Storage::DBI/connect_info> to turn on foreign key
