# PODNAME: DBIO::Storage::DBI::Replicated::Introduction
# ABSTRACT: Minimum Need to Know
=head1 SYNOPSIS

This is an introductory document for L<DBIO::Storage::DBI::Replicated>.

This document is not an overview of what replication is or why you should be
using it. It is not a document explaining how to set up MySQL native replication
either. Copious external resources are available for both. This document
presumes you have the basics down.

=head1 DESCRIPTION

L<DBIO> supports a framework for using database replication. This system
is fully integrated, which means once it is set up you should be able to
start using a replication cluster automatically, without additional work or
...
a transaction, right?) however in our experience properly written DBIO code will
work transparently with Replicated storage.

Currently we support MySQL native replication, which is relatively
easy to install and configure. We currently support a single master and one
or more replicants (also called replicas, and historically called
'slaves' in some documentation). However the framework is not specifically tied
to MySQL, and supporting other
...
By default, when you start L<DBIO>, your Schema (L<DBIO::Schema>)
...
L<DBIO::MySQL::Storage>. Your storage type class will contain
database specific code to help smooth over the differences between databases
and let L<DBIO> do its thing.
...
and DBIO will automatically adjust itself accordingly.
...
storage itself (L<DBIO::Storage::DBI::Replicated>).  A replicated storage
takes a pool of replicants (L<DBIO::Storage::DBI::Replicated::Pool>)
and a software balancer (L<DBIO::Storage::DBI::Replicated::Balancer>).
...
it possible to create a system that automatically scales under load by creating
more replicants as needed, perhaps using a cloud system such as Amazon EC2.
However, for common use you can set up your replicated storage to be enabled at
...
using (or upgrading to) the latest L<Catalyst::Model::DBIC::Schema>, which makes
...
Then, you need to connect your L<DBIO::Schema>.

  $schema->connection($dsn, $user, $pass);

Let's break down the settings.  The method L<DBIO::Schema/storage_type>
...
replicated storage.  L<DBIO::Storage::DBI::Replicated/balancer_type> is
...
L<DBIO::Storage::DBI::Replicated::Balancer::First>,
L<DBIO::Storage::DBI::Replicated::Balancer::Random>.
...
This object (L<DBIO::Storage::DBI::Replicated::Pool>) manages all the
...
These replicants should be configured as replicas (historically called
slaves) to the master using the instructions for MySQL native replication,
