...
BEGIN {
  use DBIO;
  die('The following modules are required for Replication ' . DBIO::Optional::Dependencies->req_missing_for ('replicated') . "\n" )
    unless DBIO::Optional::Dependencies->req_ok_for ('replicated');
}

use Moose;
use DBIO::Storage::DBI;
use DBIO::Storage::DBI::Replicated::Pool;
use DBIO::Storage::DBI::Replicated::Balancer;
use DBIO::Storage::DBI::Replicated::Types qw/BalancerClassNamePart DBICSchema DBICStorageDBI/;
...
L<DBIO::Storage::DBI::Replicated::Pool> and the replicants attribute for more.
...
Basically, any method request that L<DBIO::Storage::DBI> would normally
...
has 'pool_type' => (
  is=>'rw',
  isa=>ClassName,
  default=>'DBIO::Storage::DBI::Replicated::Pool',
  handles=>{
    'create_pool' => 'new',
  },
);
...
has 'balancer_type' => (
  ...
  coerce=>1,
  required=>1,
  default=> 'DBIO::Storage::DBI::Replicated::Balancer::First',
  handles=>{
    'create_balancer' => 'new',
  },
);
...
has 'pool' => (
  is=>'ro',
  isa=>'DBIO::Storage::DBI::Replicated::Pool',
  lazy_build=>1,
  handles=>[qw/
    connect_replicants
    replicants
...
has 'balancer' => (
  is=>'rw',
  isa=>'DBIO::Storage::DBI::Replicated::Balancer',
  lazy_build=>1,
...
my $method_dispatch = {
  writer => [qw/
    ...
    _do_query
    _dbh_execute
  /, Class::MOP::Class->initialize('DBIO::Storage::DBIHacks')->get_method_list ],
  reader => [qw/
    select
    select_single
    columns_info_for
  ...
  /,(
    ...
    grep
      ...
      ( Class::MOP::Class->initialize('DBIO::Storage::DBI')->get_all_method_names )
  )],
};

if (DBIO::_ENV_::DBICTEST) {
  ...
  if (my @cant = grep { ! DBIO::Storage::DBI->can($_) } keys %$seen) {
    die(join "\n", '',
...
around connect_info => sub {
  ...
  return preserve_context {
    ...
    DBIO::Storage::DBI::Replicated::WithDSN->meta->apply($master);
...
sub _build_master {
  my $self = shift @_;
  my $master = DBIO::Storage::DBI->new($self->schema);
  $master
}
...
L<connect_info|DBIO::Storage::DBI/connect_info>
...
L<< $result->discard_changes|DBIO::Row/discard_changes >>)
