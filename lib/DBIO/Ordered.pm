...
# keep in mind that you can not invoke L<DBIO::Row/update> like
...
L<DBIO::Row> methods only. If you use the
L<DBIO::ResultSet> versions of
L<update|DBIO::ResultSet/update> or
L<delete|DBIO::ResultSet/delete>, all logic present in this
module will be bypassed entirely (possibly resulting in a broken
order-tree). Instead always use the
L<update_all|DBIO::ResultSet/update_all> and
L<delete_all|DBIO::ResultSet/delete_all> methods, which will
invoke the corresponding L<row|DBIO::Row> method on every
...
and using transactions (see L<DBIO::Storage/txn_do>)
...
you will need to call L<DBIO::Row/discard_changes> to get an object
