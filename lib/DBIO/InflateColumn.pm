...
through to L<SQL::Abstract>. This is to allow setting raw
...
something else, see L<DBIO::FilterColumn>.
...
sub inflate_column {
  ...
  $self->throw_exception("InflateColumn can not be used on a column with a declared FilterColumn filter")
    if defined $colinfo->{_filter_info} and $self->isa('DBIO::FilterColumn');
...
analogous to L<DBIO::Row/get_column> in that it only fetches a
...
analogous to L<DBIO::Row/set_column>.
...
as dirty. This is directly analogous to L<DBIO::Row/store_column>.
...
=item L<DBIO::Core> - This component is loaded as part of the
