package DBIO::CDBICompat;

use strict;
use warnings;
use base qw/DBIO::Core DBIO::DB/;

# Modules CDBICompat needs that DBIO does not.
my @Extra_Modules = qw(
    Class::Trigger
...
DBIO::CDBICompat - Class::DBI Compatibility layer.
...
DBIO features a fully featured compatibility layer with L<Class::DBI>
and some common plugins to ease transition for existing CDBI users.

This is not a wrapper or subclass of DBIO but rather a series of plugins.  The result being that even though you're using the Class::DBI emulation layer you are still getting DBIO objects.  You can use all DBIO features and methods via CDBICompat.  This allows you to take advantage of DBIO features without having to rewrite your CDBI code.


=head2 Plugins

CDBICompat is good enough that many CDBI plugins will work with CDBICompat, but many of the plugin features are better done with DBIO methods.
