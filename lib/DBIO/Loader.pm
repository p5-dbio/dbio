package DBIO::Loader;
# ABSTRACT: Dynamic schema introspection and Result class generation

use strict;
use warnings;
use base qw/DBIO::Schema Class::Accessor::Grouped/;
use MRO::Compat;
use mro 'c3';
use Carp::Clan qw/^DBIO/;
use Scalar::Util 'weaken';
use Sub::Util 'set_subname';
use DBIO::Loader::Utils qw/array_eq sigwarn_silencer/;
use Try::Tiny;
use curry;
use namespace::clean;
...
=head1 SYNOPSIS
    ...
    # in a script
    use DBIO::Loader qw/ make_schema_at /;
    make_schema_at(
        'My::Schema',
        { debug => 1,
          dump_directory => './lib',
    ...
    # from the command line or a shell script with dbiodump (distributed
    # with this module).  Do `perldoc dbiodump` for usage.
...
L<DBIO::Loader::Base> and L<DBIO::Loader::DBI>.
...
If the classname starts with "::", "DBIO::Loader" is
prepended. Defaults to L<DBIO::Schema/storage_type> (which must
start with "::" when using L<DBIO::Loader>).
...
L<DBIO::Loader::Base> documentation.
...
sub _invoke_loader {
    ...
    if ($loader_class) {
        $loader_class = "DBIO::Loader${loader_class}" if $loader_class =~ /^::/;
        $args->{loader_class} = $loader_class;
    };

    # XXX this only works for relative storage_type, like ::DBI ...
    my $impl = $loader_class || "DBIO::Loader" . $self->storage_type;
    try {
...
See L<DBIO::Schema/connection> for basic usage.
...
sub connection {
    ...
    # before connecting.
    require DBIO::Loader::Base;
    my $temp_loader = DBIO::Loader::Base->new(
        %{ $self->_loader_args },
        schema => $self,
        naming => 'current',
        use_namespaces => 1,
...
Calling this as a class method on either L<DBIO::Loader>
...
See L<DBIO::Loader::Base/dump_directory> for more
details on the dumping mechanism.

This can also be set at module import time via the import option
C<dump_to_dir:/foo/bar> to L<DBIO::Loader>, where
C</foo/bar> is the target directory.

Examples:

    # My::Schema isa DBIO::Loader, and has connection info
    #   hardcoded in the class itself:
    perl -MDBIO::Loader=dump_to_dir:/foo/bar -MMy::Schema -e1

    # Same, but no hard-coded connection, so we must provide one:
    perl -MDBIO::Loader=dump_to_dir:/foo/bar -MMy::Schema -e 'My::Schema->connection("dbi:Pg:dbname=foo", ...)'
    ...
    # Or as a class method on the DBIO::Loader itself, which affects all
    #   derived schemas
    use My::Schema;
    use My::OtherSchema;
    DBIO::Loader->dump_to_dir('/foo/bar');
    My::Schema->connection(.......);
    My::OtherSchema->connection(.......);

    # Another alternative to the above:
    use DBIO::Loader qw| dump_to_dir:/foo/bar |;
    use My::Schema;
    use My::OtherSchema;
    My::Schema->connection(.......);
    My::OtherSchema->connection(.......);
...
This function creates a DBIO schema from an existing RDBMS
schema.  With the C<dump_directory> option, generates a set of
DBIO classes from an existing database schema read from the
...
L<DBIO::Loader::Base/naming> for details.
...
L<DBIO::Loader::Base/use_namespaces> for details.
...
See L<DBIO::Loader::Base/db_schema>.
