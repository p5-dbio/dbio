...
use constant SPURIOUS_VERSION_CHECK_WARNINGS => (
  ( $ENV{DBIO_TEST_VERSION_WARNS_INDISCRIMINATELY} || $ENV{DBICTEST_VERSION_WARNS_INDISCRIMINATELY} || $] < 5.010 )
    ? 1
    : 0
);
...
BEGIN {
  ...
  use constant {
    ...
    PEEPEENESS => ( $INC{"DBICTest/RunMode.pm"} && eval { DBICTest::RunMode->is_smoker } && ($] >= 5.013005 and $] <= 5.013006) ),

    SHUFFLE_UNORDERED_RESULTSETS => ($ENV{DBIO_SHUFFLE_UNORDERED_RESULTSETS} || $ENV{DBIC_SHUFFLE_UNORDERED_RESULTSETS}) ? 1 : 0,

    ASSERT_NO_INTERNAL_WANTARRAY => ($ENV{DBIO_ASSERT_NO_INTERNAL_WANTARRAY} || $ENV{DBIC_ASSERT_NO_INTERNAL_WANTARRAY}) ? 1 : 0,

    ASSERT_NO_INTERNAL_INDIRECT_CALLS => ($ENV{DBIO_ASSERT_NO_INTERNAL_INDIRECT_CALLS} || $ENV{DBIC_ASSERT_NO_INTERNAL_INDIRECT_CALLS}) ? 1 : 0,

    STRESSTEST_UTF8_UPGRADE_GENERATED_COLLAPSER_SOURCE => ($ENV{DBIO_STRESSTEST_UTF8_UPGRADE_GENERATED_COLLAPSER_SOURCE} || $ENV{DBIC_STRESSTEST_UTF8_UPGRADE_GENERATED_COLLAPSER_SOURCE}) ? 1 : 0,
