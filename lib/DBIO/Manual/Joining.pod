# PODNAME: DBIO::Manual::Joining
# ABSTRACT: Manual on joining tables with DBIO
=head1 DESCRIPTION

This document should help you to use L<DBIO> if you are trying
to convert your normal SQL queries into DBIO based queries, if
...
then you should likely try the L<DBIO::Manual::Intro>
...
L<https://dev.mysql.com/doc/refman/en/join.html>.

=head1 DEFINING JOINS AND RELATIONSHIPS

In L<DBIO> each relationship between two tables needs to first
be defined in the L<ResultSource|DBIO::Manual::Glossary/ResultSource> for the
...
comprehensively described in L<DBIO::Relationship>.
...
L<DBIO::Relationship> docs.

When performing either a L<search|DBIO::ResultSet/search> or a
L<find|DBIO::ResultSet/find> operation, you can specify which
C<relations> to also refine your results based on, using the
L<join|DBIO::ResultSet/join> attribute, like this:
...
read L<DBIO::ResultSet/search> and
L<DBIO::ResultSet/ATTRIBUTES>, but here's a quick break down:
...
output, see the L<DBIO::Manual::FAQ> for an explanation.
...
As an aside to all the discussion on joins, note that L<DBIO>
