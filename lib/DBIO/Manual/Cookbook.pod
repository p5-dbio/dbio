# PODNAME: DBIO::Manual::Cookbook
# ABSTRACT: Miscellaneous recipes
=head1 SEARCHING

=head2 Paged results

When you expect a large number of results, you can ask L<DBIO> for a
...
You can get a L<DBIO::ResultSet::Pager> object for the resultset
...
can be easily done with L<< $rs->first|DBIO::ResultSet/first
...
object.  L<< $rs->single|DBIO::ResultSet/single >> is designed
...
get the results as a L<DBIO::ResultSet>.

This is accomplished by defining a
L<ResultSource::View|DBIO::ResultSource::View> for your query,
...
... and you'll get back a perfect L<DBIO::ResultSet> (except, of course,
that you cannot modify the rows it contains, e.g. cannot call
L<update|DBIO::ResultSet/update> or
L<delete|DBIO::ResultSet/delete> on it).
...
L<DBIO::Schema/deploy>, add this line to your source definition, so that
...
L<DBIO::ResultSet/ATTRIBUTES>). You can control the C<AS> part of the
...
L<add_columns|DBIO::ResultSource/add_columns>), you just access it as
...
L<DBIO> supports C<GROUP BY> as follows:
...
Please see L<DBIO::ResultSet/ATTRIBUTES> documentation if you
are in any way unsure about the use of the attributes above (C< join
>, C< select >, C< as > and C< group_by >).

=head2 Subqueries

You can write subqueries relatively easily in DBIO.
...
If you're using L<DBIO::Schema/load_namespaces>, simply place the file
into the C<ResultSet> directory next to your C<Result> directory, and it will
be automatically loaded.

If however you are still using L<DBIO::Schema/load_classes>, first tell
DBIO to create an instance of the ResultSet class for you, in your
My::DBIO::Schema::CD class:

  # class definition as normal
  use base 'DBIO::Core';
  __PACKAGE__->table('cd');

  # tell DBIO to use the custom ResultSet class
  __PACKAGE__->resultset_class('My::DBIO::ResultSet::CD');
...
it can be accomplished with C<DBIO> when necessary by resorting to
literal SQL:
...
explained in L<DBIO::ResultSet/DBIO BIND VALUES>.
...
and L<GenericSubQ|DBIO::SQLMaker::LimitDialects/GenericSubQ> is either
too slow or does not work at all, you can try the
L<software_limit|DBIO::ResultSet/software_limit>
L<DBIO::ResultSet> attribute, which skips over records to simulate limits
in the Perl layer.
...
ODBC driver does not have proper cursor support (i.e. it loads the whole
...
This requires that you have defined the L<DBIO::Relationship>. For example :
...
(see L<DBIO::Manual::Joining> for more details on aliasing ).
...
the artist name for any of the CD objects returned, L<DBIO> will go back
to the database:
...
Thankfully, L<DBIO> has a C<prefetch> attribute to solve this problem.
...
L<DBIO> has now prefetched all matching data from the C<artist> table,
...
=head2 Static sub-classing DBIO result classes
...
B<Result-Subclass definition>
    ...
    # otherwise DBIO::ResultSourceProxy::Table will not be called
...
=head2 Dynamic Sub-classing DBIO proxy classes

AKA multi-class object inflation from one table

L<DBIO> classes are proxy classes, therefore some different
techniques need to be employed for more than basic subclassing.  In
this example we have a single user table that carries a boolean bit
for admin.  We would like to give the admin users
objects (L<DBIO::Row>) the same methods as a regular user but
...
L<DBIO::ResultSet> when inflating a result from storage.  So we
...
Alternatively you can use L<DBIO::DynamicSubclass> that implements
exactly the above functionality.

=head2 Skip result object creation for faster results

DBIO is not built for speed, it's built for convenience and
ease of use, but sometimes you just need to get the data, and skip the
fancy objects.

To do this simply use L<DBIO::ResultClass::HashRefInflator>.
...
L<DBIO::ResultSet/result_class> will replace any existing class
completely including any special components loaded using
load_components, eg L<DBIO::InflateColumn::DateTime>.

=head2 Get raw data for blindingly fast results

If the L<HashRefInflator|DBIO::ResultClass::HashRefInflator> solution
above is not fast enough for you, you can use a DBIO to return values
...
use the L<DBIO::ResultSet/select> attribute of L<DBIO::ResultSet/search> to force ordering).

=head1 RESULTSET OPERATIONS

=head2 Getting Schema from a ResultSet

To get the L<DBIO::Schema> object from a ResultSet, do the following:
...
ways, the obvious one is to use search:
  ...
  my $rs = $schema->resultset('Items')->search(
    ...
    {
       select => [ { sum => 'Cost' } ],
       as     => [ 'total_cost' ], # remember this 'as' is for DBIO::ResultSet not SQL
    }
  );
  my $tc = $rs->first->get_column('total_cost');

Or, you can use the L<DBIO::ResultSetColumn>, which gets
...
See L<DBIO::ResultSetColumn> for more documentation.
...
L<set_cache|DBIO::ResultSet/set_cache> method:
...
This is straightforward using L<ManyToMany|DBIO::Relationship/many_to_many>:
...
Mapping relationships across L<DB schemas|DBIO::Manual::Glossary/DB schema>
...
L<connection|DBIO::Schema/connection> method in your Schema class and
...
By overriding the L<connection|DBIO::Schema/connection>
...
L<connection|DBIO::Schema/connect>, as follows:
...
L<DBIO::Storage> and L<DBIO::Schema>.  Here is an
...
L<DBIO::Schema::Loader> will connect to a database and create a
L<DBIO::Schema> and associated sources by examining the database.

The recommend way of achieving this is to use the L<dbiodump> utility or the
L<Catalyst> helper, as described in
L<Manual::Intro|DBIO::Manual::Intro/Using DBIO::Schema::Loader>.

Alternatively, use the
L<make_schema_at|DBIO::Schema::Loader/make_schema_at> method:

  perl -MDBIO::Schema::Loader=make_schema_at,dump_to_dir:./lib \
...
In case you're going to use this "trick" together with L<DBIO::Schema/deploy> or
L<DBIO::Schema/create_ddl_dir> a table called "dual" will be created in your
...
Or use L<DBIO::ResultClass::HashRefInflator>
...
L<callback system|DBIO::ResultSource/sqlt_deploy_callback> if you wish
...
The following example shows simplistically how you might use DBIO to
...
Call L<DBIO::Schema/create_ddl_dir> as above under L</Creating DDL SQL>.
...
DBIO, export the modified schema and the conversion scripts as
in L</Creating DDL SQL>.

B<Deploy update to customers>

Add the L<DBIO::Schema::Versioned> schema component to your
...
L<DBIO::ResultSet/create> and L<DBIO::Row/update> family of
methods:
...
want to move by hand. Try this nifty script instead:
  ...
  $translator->producer('SQL::Translator::Producer::DBIO::File');
...
L<DBIO> uses the L<Class::C3> package, which provides for redispatch of
...
L<Class::C3> with L<DBIO> can be found in
L<DBIO::Manual::Component>.
...
documentation. See also L<DBIO::Manual::Component> for more
ways to write your own base classes to do this.

People looking for ways to do "triggers" with DBIO are probably
...
In DBIO, we'll overload the column accessor to provide a sane default if no
...
=head2 DBIO objects with Data::Dumper
...
When you enable L<DBIO::Storage>'s debugging it prints the SQL
executed as well as notifications of query completion and transaction
begin/commit.  If you'd like to profile the SQL you can subclass the
L<DBIO::Storage::Statistics> class and write your own profiling
mechanism:
...
You might want to check out L<DBIO::QueryLog> as well.
...
L<DBIO|DBIO> programs can have a significant startup delay
...
If you are using
L<DBIO::Schema::Loader|DBIO::Schema::Loader> to build the
...
using L<DBIO::Schema::Loader> to dump the database schema once - see
L<make_schema_at|DBIO::Schema::Loader/make_schema_at> and
L<dump_directory|DBIO::Schema::Loader::Base/dump_directory> for more
details on creating static schemas from a database).

=head2 Move Common Startup into a Base Class

Typically L<DBIO> result classes start off with
...
If you are instead using the L<load_namespaces|DBIO::Schema/load_namespaces>
...
L<DBIO> normally caches all statements with
