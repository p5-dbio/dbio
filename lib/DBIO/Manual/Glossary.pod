# PODNAME: DBIO::Manual::Glossary
# ABSTRACT: Clarification of terms used.
=head1 INTRODUCTION

This document lists various terms used in DBIO and attempts to
explain them.
...
to objects in programming-language-space. DBIO is an ORM.

=head2 Relationship

In DBIO a relationship defines the connection between exactly
...
At least one L<DBIO::Schema> class is needed per database.
...
in your application, they should inherit from L<DBIO::Core>.

See also: L<DBIO::Manual::ResultClass>
...
See also: L<DBIO::ResultSource/METHODS>
...
can be found in L<the FAQ|DBIO::Manual::FAQ>.

=head2 Related data

In SQL, related data actually refers to data that are normalised into
the same table. (Yes. DBIO does mis-use this term.)
