...
use DBIO::Carp qw/^SQL::Translator|^DBIO|^Try::Tiny/;
use DBIO::Exception;
...
# We're working with DBIO Schemas, not data streams.
# -------------------------------------------------------------------
sub parse {
    my ($tr, $data)   = @_;
    ...
    for (qw(DBIO::Schema DBIx::Schema package)) {
    ...
    DBIO::Exception->throw('No DBIO::Schema') unless ($dbicschema);

    if (!ref $dbicschema) {
      eval "require $dbicschema"
        or DBIO::Exception->throw("Can't load $dbicschema: $@");
    }
    ...
    for my $moniker (@monikers){
      my $source = $dbicschema->source($moniker);
       if ( $source->isa('DBIO::ResultSource::Table') ) {
         $table_monikers{$moniker}++;
      } elsif( $source->isa('DBIO::ResultSource::View') ){
...
sub _resolve_deps {
    ...
    if ( blessed($question)
        && $question->isa('DBIO::ResultSource::View') )
    {
...
C<SQL::Translator::Parser::DBIO> reads a DBIO schema,
interrogates the columns, and stuffs it all in an $sqlt_schema object.

Its primary use is in deploying database layouts described as a set
of L<DBIO> classes, to a database. To do this, see
L<DBIO::Schema/deploy>.

This can also be achieved by having DBIO export the schema as a
...
The DBIO schema (either an instance or a class name) to be parsed.
...
L<SQL::Translator>, L<DBIO::Schema>
