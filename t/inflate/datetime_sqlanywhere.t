...
plan skip_all => 'Test needs ' .
  (join ' and ', map { $_ ? $_ : () }
    ...
    (join ' or ', map { $_ ? $_ : () }
      DBIO::Optional::Dependencies->req_missing_for('test_rdbms_sqlanywhere'),
      DBIO::Optional::Dependencies->req_missing_for('test_rdbms_sqlanywhere_odbc')))
  unless
    DBIO::Optional::Dependencies->req_ok_for ('test_dt') && (
    $dsn && DBIO::Optional::Dependencies->req_ok_for('test_rdbms_sqlanywhere')
    or
    $dsn2 && DBIO::Optional::Dependencies->req_ok_for('test_rdbms_sqlanywhere_odbc'))
      or (not $dsn || $dsn2);
