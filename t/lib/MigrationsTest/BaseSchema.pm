...
use strict;
use warnings;
use base qw(MigrationsTest::Base DBIx::Class::Schema);
...
sub capture_executed_sql_bind {
  ...
  require MigrationsTest::SQLTracerObj;
  ...
  local $self->storage->{debugobj} = my $tracer_obj = MigrationsTest::SQLTracerObj->new;
...
# just pretend we are python you know? :)
  ...
  if (
    ...
    if ($locktype and (!$locker or $locker->{type} ne $locktype)) {
      ...
      my $lockpath = MigrationsTest::RunMode->tmpdir->file("_dbictest_$locktype.lock");
